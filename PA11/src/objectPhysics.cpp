...
void ObjectPhysics::setRigidBody(glm::mat4 model, std::string motionType) {
	//keep track of this
	objectType = motionType;
	if (objectRigidBody==NULL) {
        btTransform temp;
        temp.setFromOpenGLMatrix(glm::value_ptr(model));
		btDefaultMotionState* objectMotionState = new btDefaultMotionState(temp);
		...
		if (motionType == "ground") {
	        ...
	        objectRigidBody->setAnisotropicFriction(btVector3(0.2,0.2,0.2));
            objectRigidBody->setRestitution(0.3);
            ...
            //objectRigidBody->setAnisotropicFriction(btVector3(0.2,0.2,0.2));
	        objectRigidBody->setRestitution(0.3);//between 0 and 1: 0 no bounce, 1 strong bounce
...
void ObjectPhysics::setMotion(glm::vec3 scaler, glm::mat4 model, glm::vec3 inertia) {
	//set all values
	//if (objectType == "dynamic") {
           objectShape->setLocalScaling(btVector3(scaler.x, scaler.y, scaler.z));
    //}
...
void ObjectPhysics::resetMotion(glm::mat4 model, glm::vec3 scaler) {
    ...
    objectRigidBody->getCollisionShape()->setLocalScaling(btVector3(scaler.x, scaler.y, scaler.z));
