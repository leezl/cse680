...
Mesh::Mesh(glm::vec3 maxi, glm::vec3 mini, std::string newName) {
	...
	meshes.push_back(SubMesh());
    //vertices
    meshes[0].vertices.push_back(maxi);//1,1,1                                 //0
    meshes[0].vertices.push_back(glm::vec3(maxi.x, maxi.y, mini.z));//1,1,-1   //1
    meshes[0].vertices.push_back(glm::vec3(mini.x, maxi.y, mini.z));//-1,1,-1  //2
    meshes[0].vertices.push_back(glm::vec3(mini.x, maxi.y, maxi.z));//-1,1,1   //3
    meshes[0].vertices.push_back(mini);////-1,-1,-1                            //4
    meshes[0].vertices.push_back(glm::vec3(mini.x, mini.y, maxi.z));//-1,-1,1  //5
    meshes[0].vertices.push_back(glm::vec3(maxi.x, mini.y, maxi.z));//1,-1,1   //6
    meshes[0].vertices.push_back(glm::vec3(maxi.x, mini.y, mini.z));//1,-1,-1  //7

    //normals
    meshes[0].normals.push_back(glm::vec3(1,1,1));
    meshes[0].normals.push_back(glm::vec3(1,1,-1));
    meshes[0].normals.push_back(glm::vec3(-1,1,-1));
    meshes[0].normals.push_back(glm::vec3(-1,1,1));
    meshes[0].normals.push_back(glm::vec3(-1,-1,-1));
    ...
    meshes[0].indices.push_back(0);
    meshes[0].indices.push_back(1);
    meshes[0].indices.push_back(2);//end top face 1
    meshes[0].indices.push_back(0);
    meshes[0].indices.push_back(2);
    meshes[0].indices.push_back(3);//end 2nd top face

    meshes[0].indices.push_back(4);
    meshes[0].indices.push_back(6);
    meshes[0].indices.push_back(5);//end bottom face 1
    meshes[0].indices.push_back(4);
    meshes[0].indices.push_back(7);
    meshes[0].indices.push_back(6);//end bottom face 2nd

    meshes[0].indices.push_back(0);                 //max.x
    meshes[0].indices.push_back(7);                 //max.x
    meshes[0].indices.push_back(1);//right Side 1st //max.x
    meshes[0].indices.push_back(0);
    meshes[0].indices.push_back(6);
    meshes[0].indices.push_back(7);//right Side 2nd

    meshes[0].indices.push_back(4);                 //min.x
    meshes[0].indices.push_back(3);                 //min.x
    meshes[0].indices.push_back(2);//left Side 1st  //min.x
    meshes[0].indices.push_back(4);
    meshes[0].indices.push_back(5);
    meshes[0].indices.push_back(3);//left Side 2nd

    /*meshes[0].indices.push_back();                 //max.z
    meshes[0].indices.push_back();                 //max.z
    meshes[0].indices.push_back();//back Side 1st //max.z
    meshes[0].indices.push_back();
    meshes[0].indices.push_back();
    meshes[0].indices.push_back();//back Side 2nd

    meshes[0].indices.push_back();                 //min.z
    meshes[0].indices.push_back();                 //min.z
    meshes[0].indices.push_back();//front Side 1st  //min.z
    meshes[0].indices.push_back();
    meshes[0].indices.push_back();
    meshes[0].indices.push_back();//front Side 2nd*/
...
Mesh::Mesh(std::string path, std::string filename) {
    ...
    totalVerts=0;
    totalTriangles=0;
    brightness = glm::vec4(1.0,1.0,1.0,0.0);
	//loading object
    if ( !loadAssImp(path, filename) ) {
        printf("Error Loading Object File with Assimp.\n");
        exit(-1);
    }
    setBoard(); //will do nothing if no config file match
}

/*
Sets board parameters: could load from file, for now setting here by hand.
*/
void Mesh::setBoard() {
    if (name == "assets/models/squareMaze3NoNorm.obj" || name == "assets/models/squareMaze3NoNormKnobs.obj") {
        board.stopLocation = glm::vec3(3.2, 0.2, -2.5);
        board.startLocation = glm::vec3(-3.2, 0.2, 3.2);
        board.startLocation2 = glm::vec3(0.5, 0.2, -2.5);
    } else if (name == "assets/models/board.obj") {
        board.startLocation = glm::vec3(0.0, 0.1, 0.0);
        board.stopLocation = glm::vec3(-1.0, 0.1, 4.5);
        board.startLocation2 = glm::vec3(1.0, 0.1, 4.5);
    } else if (name == "assets/models/roundMaze1NoNorm.obj" || name == "assets/models/roundMaze1NoNormKnobs.obj") {
        std::cout<<"NOTICE getting round maze params"<<std::endl;
        board.startLocation = glm::vec3(-2.4, 0.4, -2.9);
        board.stopLocation = glm::vec3(0.0, 0.4, 0.0);
        board.startLocation2 = glm::vec3(2.2, 0.4, -3.2);
    }
    //add more board parameters here
...
bool Mesh::loadAssImp(std::string dir, std::string path){
    ...
    importer.SetPropertyFloat(AI_CONFIG_PP_GSN_MAX_SMOOTHING_ANGLE,89.53f);

    // read file into scene object
    // possible flags:
    // triangles, and normal important. others probably won't do much (vertex normals)//| aiProcess_JoinIdenticalVertice//
    const aiScene *scene = importer.ReadFile(path.c_str(), aiProcess_Triangulate | aiProcess_OptimizeMeshes | aiProcess_GenSmoothNormals );//SmoothNormals?
    ...
    for (unsigned int j = 0; j<scene->mNumMeshes; j++) {
        ...
        //grab normals
        if (mesh->HasNormals()) {
            //std::cout<<"Loading Normals "<<name<<std::endl;
            ...
            std::cout<<"Model has no normals, not even generated ones. "<<name<<std::endl;
        }
    ...
    if (scene->HasMaterials()) {
        ...
        for (unsigned int i = 0; i<scene->mNumMaterials; i++) {
            ...
            for (unsigned int texs=0; texs<texTypes.size(); texs++) {
                //std::cout<<"aiTextureType "<<texTypes[texs]<<','<<name<<std::endl;
...
bool Mesh::setProgram(std::vector< Program* > programs) {
	...
	for (unsigned int i = 0; i<meshes.size(); i++) {
		...
		if (meshes[i].normals.size()>0 && meshes[i].hasTextures) {
            //std::cout<<name<<" has textures and normals"<<std::endl;
			//check each program
            ...
            //std::cout<<name<<" has normals"<<std::endl;
			//check each program
			for (unsigned int j=0; j<programs.size(); j++) {
				if (programs[j]->needsNormals && !programs[j]->needsTextures) {
				    meshes[i].program = programs[j]; 
...
bool Mesh::setProgram( Program* program) {
    //select the program that suits each SubMesh
    for (unsigned int i = 0; i<meshes.size(); i++) {
        //check SubMesh parameters and program Parameters
        meshes[i].program = program;
    }
}

void Mesh::addPhysics(std::string shapeType, std::string motionType, PhysicsWorld* world, glm::mat4 model, glm::vec3 scale) {
    //std::cout<<"Setting Mesh Physics "<<name<<','<<physics.size()<<std::endl;
    ...
    //set RIGIDBODY
    physics[physics.size()-1]->setRigidBody(model, motionType);
}

void Mesh::resetPhysics(glm::mat4 model, glm::vec3 scaler) {
	//for each physics object reset
	for (unsigned int i=0; i<physics.size(); i++) {
		physics[i]->resetMotion(model, scaler);
	}
}
...
void Mesh::drawMeshes(glm::mat4 model) {
	for (unsigned int i=0; i<meshes.size(); i++) {
		if (meshes[i].program->view==NULL || meshes[i].program->projection==NULL) {
	        std::cerr<<"No view, projection or light set for program. "<<meshes[i].program->name<<std::endl;
	    }
        ...
        //check for normals
        if (meshes[i].program->needsLighting && meshes[i].normals.size()>0) {
        ...
        if (meshes[i].uvs.size()>0) {
            ...
            glVertexAttribPointer(meshes[i].program->loc_uv,  // location of attribute
                              ...
                              0,//sizeof(glm::vec3),  // stride
                              (void*)0);  // offset
            checkError("after texture coord buffer bound");
        } else {
