...
Program::Program(bool useNormals/*=false*/, 
    ...
    needsLighting = useLighting;
    //std::cout<<"What program needs "<<useNormals<<','<<useColors<<','<<useTextures<<','<<useLighting<<std::endl;
    view=NULL;
    projection=NULL;
	if (compileShaders()) {
...
bool Program::compileShaders() {
	...
	if (needsNormals &&  !needsColors &&  !needsTextures && needsLighting) {// t,f,f
	...
	} else if (!needsNormals && needsColors &&  !needsTextures && !needsLighting) { // f,t,f //not implemented yet
	...
	} else if (needsNormals && !needsColors && needsTextures && needsLighting) { // t,f,t
	...
	} else if ( !needsNormals && !needsColors && needsTextures && !needsLighting ) {
        std::cout<<"Creating shader using vertices, normals and textures only."<<std::endl;
        name = "t140";
        vertex_shader = new Shader(GL_VERTEX_SHADER, "assets/shaders/t140VertShader.vs");
        fragment_shader = new Shader(GL_FRAGMENT_SHADER, "assets/shaders/t140FragShader.fs");
    } else if ( needsNormals &&  needsColors &&  !needsTextures && needsLighting) { // t,t,f //not implemented yet
    	...
    	fragment_shader = new Shader(GL_FRAGMENT_SHADER);
    } else if ( !needsNormals &&  !needsColors &&  !needsTextures && !needsLighting) { // f,f,f
        ...
        vertex_shader = new Shader(GL_VERTEX_SHADER);
        fragment_shader = new Shader(GL_FRAGMENT_SHADER);
	} else if ( needsNormals &&  needsColors &&  needsTextures && !needsLighting) { // t,t,t
...
bool Program::loadLocations() {
    ...
    //std::cout<<"Loading Locations "<<name<<std::endl;
