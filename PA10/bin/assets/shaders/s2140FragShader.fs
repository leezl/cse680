...
*/
//http://en.wikibooks.org/wiki/GLSL_Programming/GLUT/Multiple_Lights: Good SpotLight Guide Too
//#version 330 ///I wish. versions are: 1.00 ES, 1.10, 1.20, and 1.30 on laptop. BAD laptop.

varying vec4 position, N, E;// L[4];
...
void main(void){
    ...
    for (int i=0; i<8; i++) {
        ...
        //Set diffuse
        diffuse = lights[i].diffuse * material.diffuse * max(0.0, dot(N,lightDirection)) * attenuation;
        //set specular
