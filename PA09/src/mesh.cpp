...
void Mesh::drawMeshes(glm::mat4 model) {
	for (unsigned int i=0; i<meshes.size(); i++) {
		if (meshes[i].program->view==NULL || meshes[i].program->projection==NULL || meshes[i].program->light==NULL) {
	    ...
	    meshes[i].program->setLightPosition(NULL, NULL);
	    ...
	    if (meshes[i].materialIndex>=0 && materials.size()>meshes[i].materialIndex) {
            ...
            glUniform4fv(meshes[i].program->lightin.loc_AmbProd, 1, glm::value_ptr(meshes[i].program->light->amb*currentMat.amb));
            glUniform4fv(meshes[i].program->lightin.loc_SpecProd, 1, glm::value_ptr(meshes[i].program->light->spec*currentMat.spec));
            glUniform4fv(meshes[i].program->lightin.loc_DiffProd, 1, glm::value_ptr(meshes[i].program->light->diff*currentMat.diff));
            glUniform1f(meshes[i].program->lightin.loc_Shin, currentMat.shine);//ref
